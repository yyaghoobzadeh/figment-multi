...
def calc_AP_measure(scores_list):
...
def calc_mean_precision(small, e2i, numScorePerType=1, e2freq={}):
    ...
    for i in range(numtype):
        ...
        for sublist in thelist:
            ind += 1
            type_to_AP[ind].append(calc_AP_measure(sublist))
    return type_to_AP
...
if __name__ == '__main__':
    upto = -1
    cnfpath = sys.argv[1]
    exp_dir, _ = os.path.split(os.path.abspath(cnfpath))
    if '.yaml' in cnfpath: 
    ...
    (smalltest, numScorePerTypetest, etest2freq) = loadEnt2ScoresFile(matrixtest, upto, numtype, donorm)
    ...
    if False:
        ...
        print t2thrMFT
        #calcPrintMeasures(e2i_test, t2thrMFT, smallTstMFT)
        type_to_AP = calc_mean_precision(smallTstMFT, e2i_test, numScorePerType=1, e2freq=etest2freq)
        calc_MAP(type_to_AP)
        print_MFT_baseline()
    
    type_to_AP = calc_mean_precision(smalltest, e2i_test, numScorePerType=1)
    calc_MAP(type_to_AP)
