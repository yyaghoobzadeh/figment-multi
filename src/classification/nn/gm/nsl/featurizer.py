...
def wordshapefeaturizer(onename, features):
    str = re.sub(r'[a-z]+', 'a', onename)
    str = re.sub(r'[A-Z]+', 'A', str)
    str = re.sub(r'[0-9]+', '0', str)
    str = re.sub(r'[^a-zA-Z0-9 ]+', '.', str)
    str = 'SHAPE_' + str;
    features.append(str)

def tokenFeaturizer(onename, features):
    tokens = onename.split(' ')
    for tok in tokens:
        features.append('TKN_' + tok.lower())

def posFeaturizer(onename, features):
    if onename in name2pos:
        posfeas = name2pos[onename]
        features.extend(posfeas)
    else:
        toks = word_tokenize(onename)
        tok_pos_pairs = pos_tag(toks)
        for (tok, pos) in tok_pos_pairs:
            features.append('POS_' + pos)
            name2pos[onename].append('POS_' + pos)
    
def lengthFeaturizer(onename, features):
    l = len(onename)
    if l < 6:
        features.append('LENGTH_' + str(l))
    else:
        features.append('LENGTH>_5')

          

def brownClusterFeaturizer(onename, features, word2cluster):
    LEN = { 4, 8, 12, 20 }
    notfound = True
    for tok in onename.split(' '):
        if tok not in word2cluster:
            continue
        features.append('BROWN_' + str(word2cluster[tok]))


def headFeaturizer(onename, features):
    toks = word_tokenize(onename)
    tok_pos_pairs = pos_tag(toks)
    head = ''
    for (tok, pos) in tok_pos_pairs:
        if pos.startswith('N'):
            ...
            break
    features.append('HEAD_' + head)


def ngramFeaturizer(onename, features, maxn=5, normalize=False):
    if normalize:
        onename = re.sub(r'[0-9]+', '0', onename)
        onename = onename.lower()
        onename = re.sub(r'[^a-z0 ]+', '.', onename)
        
    NGRAM = 'NGRAM_'
    onename = '^' + onename + '$'
    localfea = set()
    for n in range(1, maxn):
        for i in range(len(onename) - n + 1):
            localfea.add(NGRAM + onename[i:i+n])
    features.extend(localfea)
...
def build_feature2id(all_features, outfile):
    id = 0
    fea2id = defaultdict()
    f = open(outfile, 'w')
...
def make_input_matrix(ent2fea, ent2types, fea2id, whichset='train', maxname=1, npdir='',numtype=102, t2idx=None):
