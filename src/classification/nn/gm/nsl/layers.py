...
dt = 'float64'  # @UndefinedVariable

class HiddenLayer(object):
    def __init__(self, rng, input, n_in, n_out, W=None, b=None,
                 activation=T.tanh, sparse=False):
        """
        ...
        self.b = b
        if sparse:
            lin_output = theano.sparse.basic.dot(input, self.W) + self.b
        else:
...
class OutputLayer(object):
    ...
    def __init__(self, input, n_in, n_out, sparse=False):
        ...
        if sparse:
            self.score_y_given_x = (theano.sparse.basic.dot(input, self.W) + self.b)
        else:
